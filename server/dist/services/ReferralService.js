...
export class ReferralService {
    ...
    static async updateReferralSettled(id, settled) {
    ...
    static async getReferralsByInvestorId(investorId) {
        logger.info(`ReferralService.getReferralsByInvestorId called for investorId=${investorId}`);
        const referrals = await Referral.findAll({
            where: { referrerId: investorId },
            include: ["referrer", "referred"],
            order: [["createdAt", "DESC"]],
        });
        logger.info(`Retrieved ${referrals.length} referrals for investorId=${investorId}`);
        return referrals;
    }
}
