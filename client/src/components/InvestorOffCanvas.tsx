  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <button
        ...
        className="lg:hidden fixed top-2 left-2 z-50 p-2 rounded-lg bg-green-100 border border-green-900 text-green-900 shadow-lg hover:bg-green-200 transition-colors duration-200"
      ...
      <aside
        className={`fixed lg:relative top-0 left-0 h-full z-50 transform transition-transform duration-200 ease-out
          ...
          lg:translate-x-0 w-64 sm:w-72 lg:w-64 bg-green-900 border-r border-green-700 shadow-xl lg:shadow-none`}
      >
        <nav className="h-full overflow-y-auto p-3 lg:p-4 flex flex-col">
          {/* Header - Compact on mobile */}
          <div className="mb-4 p-3 lg:p-4 border-b border-green-700">
            <UserCircleIcon className="h-8 w-8 lg:h-10 lg:w-10 text-green-100 mx-auto" />
            <h2 className="mt-2 text-center text-base lg:text-lg font-semibold text-green-100">Investor Dashboard</h2>
          </div>
            ...
            {[
              ...
              <Link
                ...
                className="flex items-center gap-3 text-green-900 p-3 rounded-lg
                         bg-green-100/10 hover:bg-green-100/20 active:bg-green-100/30 transition-colors duration-150
                         border border-transparent hover:border-green-100/30 touch-manipulation"
                onClick={handleNavClick}
              >
                <item.icon className="h-4 w-4 lg:h-5 lg:w-5 text-green-100 flex-shrink-0" />
      ...
      <main className={`flex-1 transition-none ${isOpen && !isDesktop ? 'overflow-hidden' : ''}`}>
        <div className="p-3 sm:p-4 lg:p-6 min-h-screen">
          ...
          <div className="pt-12 lg:pt-0">
            <div className="max-w-6xl mx-auto bg-white rounded-lg lg:rounded-xl shadow-sm border border-green-100 p-4 sm:p-6 lg:p-8">
              {children}
            </div>
          </div>
        </div>
